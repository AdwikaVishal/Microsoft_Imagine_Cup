            ...
            try {
                ...
                stateGroups
                    ...
                    .on('mouseenter', function (event, d) {
                        ...
                        if (count > 0) {
                            // Adjust for scroll offset or container position if needed
                            // For simplicity in React, we might just use event.pageX/Y
                            setHoveredState({
        ...
        <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 ${className}`}>
            ...
            <div className="lg:col-span-2 bg-white rounded-lg shadow-lg overflow-hidden relative border border-slate-200">
                <div className="flex flex-row items-center justify-between p-4 bg-slate-50 border-b border-slate-100">
                    ...
                    <div className="flex items-center gap-2">
                        {/* Simple Select for React (since we don't have ui/select components installed fully yet) */}
                        <select
                            value={filterType}
                            onChange={(e) => setFilterType(e.target.value)}
                            className="bg-white border rounded text-sm px-2 py-1 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                        >
                            <option value="all">All Incidents</option>
                            <option value="Flood">Floods</option>
                            <option value="Fire">Fires</option>
                            <option value="Medical">Medical</option>
                            <option value="Road Accident">Accidents</option>
                        </select>
                ...
                <div className="p-0 relative h-[500px] flex items-center justify-center bg-slate-50">
                    ...
                    {hoveredState && (
                        <div
                            className="absolute bg-slate-900/90 text-white px-3 py-2 rounded shadow-xl z-50 pointer-events-none text-xs"
                            style={{
            ...
            <div className="space-y-6">
                <div className="bg-white rounded-lg shadow-md border border-slate-200 p-4">
                    <h3 className="text-lg font-bold mb-4">Regional Summary</h3>
                        ...
                        <div className="space-y-2">
                            ...
                            {MOCK_INCIDENT_DATA.sort((a, b) => b.count - a.count).slice(0, 5).map((state, i) => (
                                <div key={i} className="flex items-center justify-between p-2 rounded hover:bg-slate-100 transition-colors cursor-default text-sm">
                                    ...
                                    <span className="font-mono font-semibold text-gray-900">{state.count}</span>
                                </div>
                            ))}
                        </div>
                    </div>
